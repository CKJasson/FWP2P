...
-(void)showAdvs{
    ...
    cellFrame.size.height = self.view.frame.size.height;	            
...
- (void)myDidSelectAdvAtIndex:(id) index
{
    ...
    if(self.delegate != nil) {
        if ([self.delegate respondsToSelector:@selector(didSelectAdvAtIndex:)]) {
            [self.delegate performSelector:@selector(didSelectAdvAtIndex:) withObject:[NSString stringWithFormat:@"%d",thumbView.tag]];
        }
    }
}    
